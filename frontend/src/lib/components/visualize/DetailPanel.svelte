...
<div class="card preset-filled-surface-100 h-[560px] flex flex-col overflow-hidden rounded-xl border shadow-sm">
	...
	<div class="p-4 space-y-3 overflow-auto flex-1 text-sm">
		...
		{#if node.parentIds?.length}<div class="bg-surface-200/60 border rounded-xl p-3"><div class="text-xs font-bold uppercase tracking-wide opacity-70">Position — parent_ids vector (fine-grained)</div><div class="text-xs font-mono mt-1 break-words">{node.parentIds.join(' → ')}</div></div>{/if}
...
{:else}<div class="card preset-filled-surface-100 h-[560px] grid place-items-center opacity-80 text-sm">Click a node → content here like original #detail-panel</div>{/if}
