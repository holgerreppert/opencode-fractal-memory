...
<div class="card preset-filled-surface-100 flex flex-col h-[560px] overflow-hidden rounded-xl border shadow-sm">
	...
	<div class="overflow-auto flex-1 divide-y divide-surface-200">
		{#each filtered as n (n.id)}<button class="w-full text-left p-3 hover:bg-surface-100 dark:hover:bg-surface-800 text-xs transition {selectedId===n.id?'preset-filled-primary-500 text-white':''}" onclick={()=> onSelect(n)}><div class="font-bold truncate">{n.label}</div><div class="opacity-80 truncate">{n.type} · L{n.level} · {n.importance?.toFixed?.(2) ?? n.importance}</div></button>{:else}<p class="p-4 opacity-80 text-sm">No nodes</p>{/each}
	</div>
</div>
