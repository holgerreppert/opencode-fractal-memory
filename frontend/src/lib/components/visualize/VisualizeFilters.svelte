...
<div class="card preset-filled-surface-100 p-3 flex flex-col lg:flex-row gap-3 items-center flex-wrap">
	<div class="relative flex-1 min-w-[220px] max-w-[420px] flex items-center shrink-0">
		<span class="absolute left-3 opacity-60 pointer-events-none">🔍</span>
		<input class="input w-full pl-9 pr-20 text-surface-900 dark:text-white bg-white dark:bg-surface-800 min-w-0" placeholder="Search memory… (⌘K) — Enter to search, Esc clear" bind:value={query} onkeydown={(e)=> { if(e.key==='Escape') query=''; if(e.key==='Enter' && query.trim()) Logger.debug('[search] enter', query); }} />
		<div class="absolute right-1 flex items-center gap-1">
			{#if query}<button class="btn btn-sm preset-tonal px-2 text-xs shrink-0" onclick={()=> { query=''; Logger.debug('[search] clear'); }}>✕</button>{/if}
			<span class="badge preset-filled-primary-500 text-[10px] px-1.5 shrink-0">{query.length ? '↩' : '/'}</span>
		</div>
	</div>
	...
	<div class="ml-auto flex gap-1 flex-wrap">
		{#each Object.entries(nodesStore.typeCounts ?? Object.fromEntries(['concept','fact','lesson','dot'].map(t=>[t, nodesStore.nodes.filter(n=>n.type===t).length]))) as [chip, cnt] (chip)}<span class="chip preset-tonal text-xs px-3 py-1"><button class="text-left w-full" onclick={()=> query=chip}>{chip}</button><span class="badge preset-filled-surface-200 text-[10px] px-1.5">{cnt}</span></span>{/each}
		{#each Object.entries(nodesStore.levelCounts ?? {}).sort((a,b)=>+a[0]-+b[0]) as [lvl, cnt] (lvl)}<span class="chip preset-tonal text-xs px-3 py-1"><button class="text-left w-full" onclick={()=> query=lvl}>L{lvl}</button><span class="badge preset-filled-surface-200 text-[10px] px-1.5 ml-1">{cnt}</span></span>{/each}
	</div>
