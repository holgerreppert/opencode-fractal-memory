...
- **Skeleton + Tailwind latest — MANDATORY whenever possible**: ALWAYS prefer `@skeletonlabs/skeleton` `5.0.1` + `@skeletonlabs/skeleton-svelte` `5.0.1` theme `cerberus` + `tailwindcss` `4.2` + `@tailwindcss/vite` `4.2` primitives over custom CSS. Check `frontend/package.json` for pinned latest before inventing styles. Use `preset-*` tokens only (`preset-filled-primary-500`, `preset-tonal`, `preset-outlined-surface-200`, `preset-filled-surface-100`, `preset-tonal-primary`), not `variantFilled` or raw hex. Workhorses: `AppShell`, `AppBar (Toolbar/Lead/Trail)`, `Accordion` (see collapsible rule), `card`, `btn btn-sm`, `badge`, `chip`, `input` (`rounded-full pl-9 pr-20 border-2 focus:border-primary-500`), `select`. Filters are horizontal pills `flex gap-1 flex-wrap`, never dropdowns. **For collapsible settings sections, ALWAYS use Skeleton `Accordion` (`Accordion` + `Accordion.Item` + `Accordion.ItemTrigger` + `Accordion.ItemContent` + `Accordion.ItemIndicator` from `@skeletonlabs/skeleton-svelte`, zag-js backed, `value=["memory"] multiple`) — never hand-rolled `collapsed` maps with `{#if}`; see `frontend/src/routes/settings/+page.svelte` canonical usage. For any other collapsible/overlay/dialog use Skeleton `Collapsible`/`Dialog`/`Popover` first, then Tailwind utilities.**
