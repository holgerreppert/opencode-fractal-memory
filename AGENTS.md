...
- **TUI sidebar (Fractal Memory box)** (`src/tui.tsx` → `dist/tui.js`, Tier-2 slots `sidebar_content` order 50, palette `/mem`): **OpenCode only scans `<project>/.opencode/tui.json` and `~/.config/opencode/tui.json` for TUI manifests — a `tui.json` inside the installed plugin package is NEVER discovered** (verified: package-root manifest shipped + synced to cache still showed no box in other projects). Registration therefore happens at install time by writing an absolute path to `dist/tui.js` into the GLOBAL `~/.config/opencode/tui.json`: `scripts/dev-install.ts` `ensureGlobalTuiRegistration()` (dev path) and `scripts/postinstall.cjs` `registerGlobalTui()` (npm path), both merge-preserving other TUI plugins. `bun run plugin:clean` strips the registration for fresh-install tests. RESTART required after any registration change.
