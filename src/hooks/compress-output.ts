...
import { memLog, writeCompressLog } from "../logging";
...
function classifyShape(raw: string): OutputShape {
...
function contentPreview(text: string, maxLen = 80): string {
  if (!text) return "";
...
function trimByRelevance(raw: string, command: string, config: CompressConfig): string {
...
export function compressCommandOutput(
  ...
  const shape = classifyShape(out);
  if (shape !== "unknown") {
    let shaped: string | null = null;
    let shapeStrategy = "";
    switch (shape) {
      case "json": shaped = compressJson(out, config.maxLines); shapeStrategy = "shape-json"; break;
      case "csv": shaped = compressCsv(out, config.maxLines); shapeStrategy = "shape-csv"; break;
      case "stack-trace": shaped = compressStackTrace(out, config.maxLines); shapeStrategy = "shape-stack"; break;
      case "tree": shaped = compressTree(out, config.maxLines); shapeStrategy = "shape-tree"; break;
      case "table": shaped = compressTable(out, config.maxLines); shapeStrategy = "shape-table"; break;
    }
    if (shaped && shaped !== out && shaped.length < out.length * 0.8) {
      memLog("debug", "compress", "Shape-based compression applied", {
        cmd: cmd.slice(0, 60), shape,
        originalChars: out.length, compressedChars: shaped.length,
        saving: `${Math.round((1 - shaped.length / Math.max(out.length, 1)) * 100)}%`,
      });
...
const DEFAULT_FUZZY: FuzzyDedupConfig = { enabled: true, similarityThreshold: 0.85, maxComparisons: 50 };
...
export function addContentDedup(
  ...
  const fuzzy = fuzzyConfig ?? DEFAULT_FUZZY;
  if (fuzzy.enabled) {
    let bestSim = 0;
    let bestKey = "";
    let count = 0;
