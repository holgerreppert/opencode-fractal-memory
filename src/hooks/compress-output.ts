...
function stripAnsiPipe(s: string): string {
...
function compressLs(raw: string): string {
  ...
  const parts: string[] = [];
  if (dirs.length > 0) {
    parts.push(`${dirs.length} dir${dirs.length !== 1 ? "s" : ""}`);
  }
  if (files.length > 0) {
...
function compressTestOutput(raw: string): string {
  ...
  for (const line of lines) {
    ...
    if (m) {
      total = Math.max(total, parseInt(m[1] ?? "0", 10));
    }
  }
  ...
  const result: string[] = [];
  if (total > 0) {
    const p = total - failed;
    result.push(`${p}/${total} passed`);
  } else {
  ...
  if (failed > 0) {
    ...
    const detail = fails.slice(0, 10).map(f => `  FAIL: ${f}`);
    if (fails.length > 10) detail.push(`  ... +${fails.length - 10} more`);
    result.push(...detail);
  }
...
function compressGrep(raw: string): string {
  ...
  if (entries.length > 15) {
    result.push(`  ... +${entries.length - 15} more files`);
  }
  return result.join("\n");
}
...
function compressGeneric(raw: string, maxLines: number): string {
  ...
  deduped.splice(maxLines);
  deduped.push(`... +${remaining} more line${remaining !== 1 ? "s" : ""}`);
  return deduped.join("\n");
}
...
function getCommandPrefix(rawCmd: string): string {
  ...
  return cmd.replace(envPrefix, "").trim();
}
