...
const COMMANDS = [
  ...
  { name: "memory_verify", desc: "Verify node correctness — boosts confidence and verification count" },
  ...
  const t = tool({
    ...
    async execute() {
      ...
      lines.push("```", "", "### Memory Categories", "",
        ...
        "",
        "### Memory Supertypes (auto-derived from type)",
        "- **Declarative** (know-that): concept, fact, knowledge, architecture, convention, research",
        "- **Procedural** (know-how): lesson, howto, rule:*, skill, playbook",
        "- **Experiential** (past events): event, note, session, task, plan, exploration, bug, debug-investigation, improvement, review",
